...
function runImplement(): Promise<number> {
    ...
    function checkForFailures() {
      for (const o of [output, errorOutput]) {
        if (o.includes('Error: No messages returned')) {
          console.log("Killing zombie process due to 'No messages returned' error.");
...
async function handleFailureSleep(failSleep: number) {
  console.log(
...
async function main() {
  ...
  while (true) {
    ...
    try {
      ...
      console.error(`\n[${formatTime()}] ❌ Error during implementation:`, err);
