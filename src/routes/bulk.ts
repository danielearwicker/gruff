...
const bulk = new OpenAPIHono<{ Bindings: Bindings }>();
...
async function findLatestEntityVersion(
  db: D1Database,
  entityId: string
...
async function findLatestLinkVersion(
  db: D1Database,
  linkId: string
): Promise<Record<string, unknown> | null> {
  const link = await db
...
bulk.openapi(bulkCreateEntitiesRoute, async c => {
  ...
  try {
    ...
    const { results: existingTypes } = await db
      .prepare(
    ...
    const typeSchemas = new Map(
      existingTypes.map((t: Record<string, unknown>) => [t.id, t.json_schema as string | null])
    );
    ...
    logger.info('Bulk create entities completed', {
      successCount,
      failureCount,
...
bulk.openapi(bulkCreateLinksRoute, async c => {
  ...
  try {
    ...
    logger.info('Bulk create links completed', {
      successCount,
      failureCount,
...
bulk.put('/entities', requireAuth(), validateJson(bulkUpdateEntitiesSchema), async c => {
  ...
  try {
    ...
    const currentVersions: Map<string, { currentVersion: Record<string, unknown>; index: number }> =
      new Map();
    ...
    if (typeIdsToFetch.size > 0) {
      ...
      const { results: types } = await db
        .prepare(`SELECT id, json_schema FROM types WHERE id IN (${typePlaceholders})`)
        .bind(...typeIdArray)
        .all();
    ...
    logger.info('Bulk update entities completed', {
      successCount,
      failureCount,
      total: data.entities.length,
    });
...
bulk.put('/links', requireAuth(), validateJson(bulkUpdateLinksSchema), async c => {
  ...
  try {
    ...
    logger.info('Bulk update links completed', {
      successCount,
      failureCount,
      total: data.links.length,
    });
